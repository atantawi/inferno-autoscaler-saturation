...
const (
	// DefaultSaturationPolicy is the default policy used when the limited capacity is saturated
	DefaultSaturationPolicy = "PriorityRoundRobin"

	// DefaultServiceClassName is the default name for the service class used in limited capacity allocation
	DefaultServiceClassName = "Default"
)

// LimiterConfiguration holds configuration for the LimitedCapacityAllocator
type LimiterConfiguration struct {
	SaturationPolicy string
	ServiceClassName string
}

// LimitedCapacityAllocator distributes the available limited amount of GPUs among variants,
// given their recommended amounts by the engine(s)
type LimitedCapacityAllocator struct {
	config *LimiterConfiguration
}
...
// NewLimitedCapacityAllocator creates a new LimitedCapacityAllocator instance.
func NewLimitedCapacityAllocator(config *LimiterConfiguration) *LimitedCapacityAllocator {
	if config == nil {
		config = &LimiterConfiguration{
	...
	if config.SaturationPolicy == "" {
		config.SaturationPolicy = DefaultSaturationPolicy
	}
	if config.ServiceClassName == "" {
		config.ServiceClassName = DefaultServiceClassName
	}
	return &LimitedCapacityAllocator{
		config: config,
	}
}

// Allocate allocates limited GPU capacity among variants by updating their respective decisions
func (l *LimitedCapacityAllocator) Allocate(
	ctx context.Context,
